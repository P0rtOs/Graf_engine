...
namespace BJEngine 
{
	...
	Camera::Camera(dx::XMVECTOR eye, dx::XMVECTOR at, dx::XMVECTOR up, float aspectRatio)
	{
		...
		camDesc.projectionMatrix = dx::XMMatrixPerspectiveFovLH(M_PI / 2.0f, aspectRatio, 1.0f, 25000.0f);
		               ...
		               FoV(M_PI / 2), cone(nullptr), moveSpeed(1.0f), sensitivity(0.001f)
	{
		...
		camDesc.projectionMatrix = dx::XMMatrixPerspectiveFovLH(M_PI / 2.0f, BJEUtils::GetWindowWidth() / BJEUtils::GetWindowHeight(), 1.0f, 25000.0f);
	...
	{
		camDesc.projectionMatrix = dx::XMMatrixPerspectiveFovLH(FoV, BJEUtils::GetWindowWidth() / BJEUtils::GetWindowHeight(), 1.0f, 25000.0f);

	...
	bool FileSave::Execute()
	{
		...
		if (file->is_open())
		{
			...
			for (const auto& el : defferedElements)
			{
				...
				(*file) << PROJECT_MODEL_SCRIPT_NAME << ' ' << ptr->GetScriptName() << '\n';
		...
		while (chk != PROJECT_SEPARATOR)
		{
			...
			if (chk == PROJECT_MODEL_START)
			{
				while (chk != PROJECT_MODEL_END)
				{
					...
					else if (chk == PROJECT_MODEL_SCRIPT_NAME)
					{
						ss >> chk;
						tmpScriptName = chk;
					}
				...
				auto el = new Element(std::move(vertices), std::move(indices), tmpPtr, dx::XMVectorSet(minExtent.x, minExtent.y, minExtent.z, 1.0f), dx::XMVectorSet(maxExtent.x, maxExtent.y, maxExtent.z, 1.0f));
				(*elVec).push_back(el);
				el->CreateScript(new Script(_strdup(tmpScriptName.c_str())));
